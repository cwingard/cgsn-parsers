...
# C. Wingard 2024-08-19 -- Updated to use a stand-alone parser for the SeaPHox
...
if [ -e "$IN_FILE" ]; then
    ...
    python -m cgsn_parsers.parsers.parse_cphox -i "$IN_FILE" -o "$OUT_FILE" || echo "ERROR: Failed to parse $IN_FILE"
fi
