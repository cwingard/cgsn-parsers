...
# harvest_turbid.sh
#
# Reads the raw Turbidity data files from the Endurance and Pioneer Surface
...
if [ -e "$IN_FILE" ]; then
    ...
    python -m cgsn_parsers.parsers.parse_turbd -i "$IN_FILE" -o "$OUT_FILE" || echo "ERROR: Failed to parse $IN_FILE"
fi
