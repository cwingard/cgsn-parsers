#!/bin/bash
# harvest_syslog_gps.sh
#
# Reads the raw GPS data in the syslog files from the Endurance and Pioneer
...
if [ -e "$IN_FILE" ]; then
    ...
    python -m cgsn_parsers.parsers.parse_syslog_gps -i "$IN_FILE" -o "$OUT_FILE" || echo "ERROR: Failed to parse $IN_FILE"
fi
