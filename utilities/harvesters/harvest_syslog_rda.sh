#!/bin/bash
# harvest_syslog_rda.sh
#
# Reads the raw RDA data in the syslog files from the Endurance and Pioneer
...
if [ -e "$IN_FILE" ]; then
    ...
    python -m cgsn_parsers.parsers.parse_syslog_rda -i "$IN_FILE" -o "$OUT_FILE" || echo "ERROR: Failed to parse $IN_FILE"
fi
