#!/bin/bash
# harvest_imm_ctdmo.sh
#
# Reads the raw CTDMO data files (telemetered via the inductive modem) from the
...
if [ -e "$IN_FILE" ]; then
    ...
    python -m cgsn_parsers.parsers.parse_imm_ctdmo -i "$IN_FILE" -o "$OUT_FILE"  || echo "ERROR: Failed to parse $IN_FILE"
fi
