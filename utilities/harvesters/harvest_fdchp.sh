...
# harvest_fdchp.sh
#
# Reads the raw FDCHP data files from the Endurance and Pioneer Surface Moorings
...
if [ -e "$IN_FILE" ]; then
    ...
    python -m cgsn_parsers.parsers.parse_fdchp -i "$IN_FILE" -o "$OUT_FILE" || echo "ERROR: Failed to parse $IN_FILE"
fi
