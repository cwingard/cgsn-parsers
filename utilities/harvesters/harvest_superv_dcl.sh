#!/bin/bash
# harvest_superv_dcl.sh
#
# Reads the raw DCL supervisor data files from the Endurance and Pioneer Surface
...
if [ -e "$IN_FILE" ]; then
    ...
    python -m cgsn_parsers.parsers.parse_superv_dcl -i "$IN_FILE" -o "$OUT_FILE" || echo "ERROR: Failed to parse $IN_FILE"
fi
