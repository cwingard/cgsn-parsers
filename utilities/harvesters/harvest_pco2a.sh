...
# harvest_pco2a.sh
#
# Reads the raw PCO2A data files from the Endurance and Pioneer Coastal Surface
...
if [ -e "$IN_FILE" ]; then
    ...
    python -m cgsn_parsers.parsers.parse_pco2a -i "$IN_FILE" -o "$OUT_FILE" || echo "ERROR: Failed to parse $IN_FILE"
fi
